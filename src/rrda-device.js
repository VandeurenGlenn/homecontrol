...
export default define(class RrdaDevice extends RenderMixin(HTMLElement) {
  ...
  get enabledButton() {
    return this.shadowRoot.querySelector('.enabled');
  }
  ...
  constructor() {
    ...
    this._enabledClick = this._enabledClick.bind(this);
  }
  ...
  connectedCallback() {
    ...
    this.enabledButton.addEventListener('click', this._enabledClick)
  ...
  _enabledClick() {
    if (this.hasAttribute('clock-enabled')) {
      ref.child(`${this.uid}/enabled`).set(0);
    } else {
      ref.child(`${this.uid}/enabled`).set(1);
    }
  }
...
//                    0 3px 5px -1px rgba(0, 0, 0, 0.4);
      ...
      :host([clock-enabled]) custom-svg-icon.enabled {
        fill: rgb(94, 146, 199);
      }
      custom-svg-icon.enabled {
        pointer-events: auto;
      }
      .row {
        display: flex;
    ...
    <span class="row title-bar">
      ...
      <custom-svg-icon icon="clock" class="enabled"></custom-svg-icon>
